...
<template>
    <div id="search">
        <div class="info">
            ...
            <router-link v-if="user.admin" v-on:click="clearSearch()" to="/plugins/search" class="active">{{ $t("search") }}</router-link>
        </div>
        <div class="content">
        ...
        methods: {
            async search() {
                if (this.query.length >= 3) {
                    ...
                    this.results = await this.api.post(`/plugins/${encodeURIComponent(this.query)}`);
