                        ...
                        <div v-for="(room, index) in available" :key="index">
                            ...
                            <div v-for="(accessory, index) in room.accessories" :key="index">
                                <div v-if="layout.rooms[current].accessories.indexOf(accessory.aid) < 0" class="available-accessory">
                                    <checkbox :id="`add-accessory-${index}`" :value="accessory.aid" v-model="selected"> <label :for="`add-accessory-${index}`">{{ accessory.hidden ? `(${$t("hidden")}) ` : "" }}{{ accessory.alias || accessory.name || accessory.service_name }}</label>
                                </div>
                            </div>
                        </div>
                    </div>
