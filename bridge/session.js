...
module.exports = class Session extends EventEmitter {
    constructor(stateChar, controlChar) {
        super();
    ...
    handleWriteRequest(request) {
        if (request.type === "Negotiate") {
            ...
            if (this.currentStage === 1) {
                ...
                this.currentPluginName = this.listOfPlatforms[request.response.selections[0]];
    ...
    presentManagePlatformMenu() {
        ...
        for (let name in this.configurablePlatformPlugins) {
    ...
    presentManageAccessoryMenu() {
        this.emit("requestCurrentConfig", (config) => {
    ...
    handleManageAccessory() {
        this.presentMainMenu();
    }

    sendResponse(response) {
        if (this.validSession) {
            const respData = new Buffer(JSON.stringify(response)).toString("base64");
