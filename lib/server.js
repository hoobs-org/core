    ...
    static saveConfig(config, skipBackup) {
        ...
        if (JSON.stringify(config) !== JSON.stringify(current)) {
            if (skipBackup) {
                ...
                File.renameSync(join(Server.paths.config, HBS.name || "", "config.json"), join(Server.paths.config, HBS.name || "", `/config-${new Date().getTime()}.json`));
            }
                        ...
                        if (line !== "") {
                            ...
                            if (line.startsWith("X-HM://")) {
                                if (HBS.config.server.home_setup_id !== line) {
                                    ...
                                    Server.saveConfig(HBS.config, true);
                                }
                            }
