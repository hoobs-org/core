...
module.exports = class Server {
    ...
    static configure() {
        ...
        if (!File.existsSync(join(Server.paths.config, global.INSTANCE || "", "config.json"))) {
            ...
            File.copyFileSync(join(Server.paths.config, "default.json"), join(Server.paths.config, global.INSTANCE || "", "config.json"));
        }
        ...
        if (init || !current.bridge.username || current.bridge.username === "") {
            if (global.INSTANCE && global.INSTANCE !== "") {
                ...
                current.server.socket = "http://hoobs.local:51828";
