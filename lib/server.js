    ...
    start() {
        return new Promise((resolve) => {
            if (!this.running) {
                ...
                HBS.log.debug(`${join(Server.paths.bridge, "/bin/bridge")} ${(this.arguments || []).join(" ")}`);

                this.proc = fork(join(Server.paths.bridge, "/bin/bridge"), this.arguments, {
                    cwd: Server.paths.application,
                    silent: true
                });
