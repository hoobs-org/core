...
module.exports = class Server {
    ...
    static restore(path) {
        global.server.stop().then(() => {
            global.log.info("[Restore System] Removing Current Plugins.");
            ...
            global.log.info("[Restore System] Reading Recovery File.");
            ...
            global.log.info("[Restore System] Unpacking Recovery File.");

            File.createReadStream(join(root, filename)).pipe(Unzip.Extract({
                path: root
            })).on("finish", () => {
                global.log.info("[Restore System] Rebooting");
