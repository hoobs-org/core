...
module.exports = class Server {
    ...
    static getIpAddress(domain) {
        if (domain) {
            return domain;
        }

        const network = OS.networkInterfaces();
        const ifaces = Object.keys(network);

        for (var i = 0; i < ifaces.length; i ++) {
            if (ifaces[i].startsWith("vnic")) {
                continue;
            }

            const iface = network[ifaces[i]];

            for (let j = 0; j < iface.length; j++) {
                if ("IPv4" !== iface[j].family || iface[j].internal !== false) {
                    continue;
                }

                return `http://${iface[j].address}`;
            }
        }
    }

    static conditionalAddress(value, address) {
        if (!value) {
            return address;
        }
        ...
        if (!Number.isNaN(parseInt(value, 10))) {
            ...
            return `${address}${value.startsWith("/") ? value : `/${value}`}`;
        }
    ...
    static async configure() {
        ...
        if (init) {
            if (global.INSTANCE && global.INSTANCE !== "") {
                ...
                current.bridge.name = `Rocket [${global.INSTANCE}]`;
        ...
        current = Server.conditionalClient(current);

        Server.saveConfig(current, true);

        const address = Server.getIpAddress(current.client.domain);

        current.client.config = Server.conditionalAddress(current.client.config, address);
        current.client.socket = Server.conditionalAddress(current.client.socket, address);

        if (!current.client.api) {
            current.client.api = [address];
        } else {
            if (Array.isArray(current.client.api)) {
                for (let i = 0; i < current.client.api.length; i++) {
                    current.client.api[i] = Server.conditionalAddress(current.client.api[i], address);
                }
            } else {
                current.client.api = [Server.conditionalAddress(current.client.api, address)];
            }
        }
    ...
    static conditionalClient(config) {
        let current = {};
        ...
        if (current.client.config) {
            config.client.config = current.client.config;
        } else if (config.client.config) {
            delete config.client.config;
        }

        if (current.client.socket) {
            config.client.socket = current.client.socket;
        } else if (config.client.socket) {
            delete config.client.socket;
        }

        if (current.client.api) {
            config.client.api = current.client.api;
        } else if (config.client.api) {
                        ...
                        if (line !== "") {
                            ...
                            if (line.startsWith("X-HM://")) {
                                if (config.server.home_setup_id !== line) {
                                    ...
                                    Server.saveConfig(Server.conditionalClient(config), true);
                                }
                            }
