...
module.exports = class Server {
    ...
    static async configure() {
        ...
        File.ensureSymlinkSync(join(Server.paths.modules.local, "@hoobs/hap"), join(Server.paths.modules.local, "hap-nodejs"));
        File.ensureSymlinkSync(join(Server.paths.modules.local, "@hoobs/homebridge"), join(Server.paths.modules.local, "homebridge"));
    ...
    static backup() {
        return new Promise((resolve, reject) => {
            ...
            output.on("close", () => {
                File.renameSync(join(Server.paths.dist, `${filename}.zip`), join(Server.paths.dist, `${filename}.hbfx`));

                resolve(`/${filename}.hbfx`);
            });
    ...
    static unpackRestore(file) {
        ...
        if (extname(file.name) === ".hbfx") {
            File.renameSync(file.path, join(root, filename));

            return join(root, filename);
        }

        if (extname(file.name) === ".hbf") {
    ...
    static checkRestore(file) {
        ...
        if (extname(file.name) === ".hbf" && File.existsSync(file.path)) {
            try {
                ...
                File.unlinkSync(join(root, filename));
    ...
    static restore(file) {
        if (!HBS.docker) {
            ...
            if (Server.checkRestore(file)) {
                HBS.server.stop().then(() => {
                    HBS.log.info("[Restore System] Removing Current Plugins.");

                    Remove.sync(Server.paths.application);

                    const filename = Server.unpackRestore(file);
