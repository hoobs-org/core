...
module.exports = class Server {
    ...
    static get paths() {
        return {
            ...
            pkg: File.existsSync("/usr/local/bin/yarn") || File.existsSync("/usr/bin/yarn") ? "yarn" : "npm"
        };
    }
    ...
    static async configure() {
        ...
        if (!File.existsSync(join(Server.paths.config, global.INSTANCE || "", "config.json"))) {
            ...
            const plugins = await Migrate.plugins(Server.paths.modules.global, Server.paths.application, Server.paths.pkg);

            if (!current.plugins || current.plugins.length === 0) {
                current.plugins = plugins;
            }

            await Migrate.cleanup(Server.paths.modules.global, Server.paths.pkg);
        } else {
            let contents = {};
