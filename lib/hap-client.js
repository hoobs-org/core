...
module.exports = class HapClient {
    ...
    services() {
        return new Promise((resolve, reject) => {
            Request.get(`${this.endpoint}/accessories?t=${new Date().getTime()}`).then((response) => {
                const services = [];
    ...
    refresh(service) {
        return new Promise((resolve, reject) => {
            ...
            Request.get(`${this.endpoint}/characteristics?id=${iids.map(iid => `${service.aid}.${iid}`).join(",")}&t=${new Date().getTime()}`).then((response) => {
    ...
    get(service, iid) {
        return new Promise((resolve, reject) => {
            Request.get(`${this.endpoint}/characteristics?id=${service.aid}.${iid}&t=${new Date().getTime()}`).then((response) => {
