            ...
            if (File.existsSync(filename)) {
                ...
                if (item.name === "homebridge" || (Array.isArray(item.keywords) && (item.keywords.indexOf("homebridge-plugin") >= 0 || item.keywords.indexOf("homebridge-interface") >= 0))) {
                    results[item.name] = {
                        ...
                        schema: {
                            platform: {
                                plugin_alias: schema.platform.plugin_alias || schema.platform.pluginAlias || item.name,
                                schema: schema.platform.schema || {}
                            },
                            accessories: {
                                plugin_alias: schema.accessories.plugin_alias || schema.accessories.pluginAlias || item.name,
