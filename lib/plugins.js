...
module.exports = class Plugins {
        ...
        for (let i = 0; i < modules.length; i++) {
            if (modules[i].startsWith("@")) {
                ...
                for (let j = 0; j < scopes.length; j++) {
                    modules.push(`${modules[i]}/${scopes[j]}`);
                }
            }
        }
    ...
    static installed() {
        ...
        return new Promise((resolve) => {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                Plugins.package(item.name, item.version).then((response) => {
    ...
    static search(search, limit) {
        return new Promise((resolve, reject) => {
            if (search && search !== "") {
                ...
                Request({
                    url: `https://api.npms.io/v2/search?q=${search}+keywords:hoobs-plugin,homebridge-plugin,hoobs-interface+not:deprecated+not:insecure&size=${limit}`,
                    json: true
                }, (error, response, body) => {
                    const results = [];
    ...
    static getPlatform(name) {
        ...
        if (File.existsSync(join(Server.paths.modules.local, `/${name}/platform.schema.json`))) {
            let schema = {};

            try {
                schema = JSON.parse(File.readFileSync(join(Server.paths.modules.local, `/${name}/platform.schema.json`)));
            } catch {
                schema = {};
            }
        ...
        } else if (File.existsSync(join(Server.paths.modules.local, `/${name}/config.schema.json`))) {
            let schema = {};

            try {
                schema = JSON.parse(File.readFileSync(join(Server.paths.modules.local, `/${name}/config.schema.json`)));
            } catch {
                schema = {};
            }
        ...
        } else if (File.existsSync(join(Server.paths.modules.local, `/${name}/config.json`))) {
            const plugin = JSON.parse(File.readFileSync(join(Server.paths.modules.local, `/${name}/config.json`)));
            ...
            process.on("close", () => {
                if (File.existsSync(join(Server.paths.modules.local, `/${name}/package.json`))) {
                    ...
                    if (item.keywords) {
                        ...
                        const platform = Plugins.getPlatform(name);

                        if (platform) {
                            config.platforms.push(push);
                        }
                    }
