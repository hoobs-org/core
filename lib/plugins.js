...
module.exports = class Plugins {
    ...
    static getPluginType(name) {
        ...
        if (File.existsSync(join(ppath, "platform.schema.json"))) {
            ...
            try {
                schema = JSON.parse(File.readFileSync(join(ppath, "platform.schema.json")));
            } catch {
                schema = {};
            }
            ...
            try {
                schema = JSON.parse(File.readFileSync(join(ppath, "accessories.schema.json")));
            } catch {
                schema = {};
            }
            ...
            try {
                schema = JSON.parse(File.readFileSync(join(ppath, "config.schema.json")));
            } catch {
                schema = {};
            }
            ...
            try {
                pjson = JSON.parse(File.readFileSync(join(ppath, "package.json")));
            } catch {
                pjson = {};
            }
    ...
    static getPlatform(id, name) {
        ...
        for (let i = 0; details.length; i++) {
            if (details[i].type === "platform") {
                const index = HBS.config.platforms.findIndex(p => p.platform === details[i].alias);
