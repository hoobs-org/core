...
module.exports = class Plugins {
    ...
    static getPlatform(name) {
        ...
        if (File.existsSync(join(Server.paths.modules.local, `/${name}/platform.schema.json`))) {
            const schema = JSON.parse(File.readFileSync(join(Server.paths.modules.local, `/${name}/platform.schema.json`)));
            const index = config.platforms.findIndex(p => p.platform === (schema || {}).pluginAlias || name);
            ...
            if (index >= 0) {
                ...
                return {
                    platform: (schema || {}).pluginAlias || name,
                    plugin_map: {
                        plugin_name: name
                    }
                };
            ...
            const schema = JSON.parse(File.readFileSync(join(Server.paths.modules.local, `/${name}/config.schema.json`)));
            const index = config.platforms.findIndex(p => p.platform === (schema || {}).pluginAlias || name);
