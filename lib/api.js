...
module.exports = class API {
    ...
    start() {
        global.app.listen(this.port, () => {
            global.log.info(`[${new Date().toLocaleString()}] API on http://localhost:${this.port}.`)

            if (global.ui) {
                global.log.info(`[${new Date().toLocaleString()}] Client on http://localhost:${config.client.port || 3002}.`);
            }

            if (!global.socketClient) {
                global.log.info(`[${new Date().toLocaleString()}] Logging on http://localhost:${config.server.socket || 3001}.`);
            }
        });
    }
}
