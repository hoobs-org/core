...
const Terminal = require("term.js-cockpit");
...
module.exports = class API {
    constructor(port) {
        ...
        this.term = Pty.fork(process.env.SHELL || "sh", [], {
            name: File.existsSync("/usr/share/terminfo/x/xterm-256color") ? "xterm-256color" : "xterm",
            cols: 80,
            rows: 24,
        ...
        this.term.on("data", (data) => {
            return !this.socket ? this.buffer.push(data) : this.socket.emit("data", data);
        });

        global.app = Express();

        this.server = Http.createServer(global.app);
        this.io = Socket(this.server);

        this.io.origins("*:*");

        this.io.sockets.on("connection", (response) => {
            ...
            while (this.buffer.length) {
                this.socket.emit("data", this.buffer.shift());
            };
        });

        global.app.use(Terminal.middleware());
