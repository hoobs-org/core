...
module.exports = class LayoutController {
    constructor() {
        this.hap = new HapClient(`http://localhost:${config.bridge.port}`, config.bridge.pin);
        this.accessories = [];
    ...
    save(request, response) {
        ...
        File.unlinkSync(Server.paths.config.layout);
        File.appendFileSync(Server.paths.config.layout, JSON.stringify(data, null, 4));
