...
module.exports = class AuthController {
    ...
    state(request, response) {
        if (global.config.system === "rocket") {
            if (global.users.filter(u => !u.admin).length === 0) {
    ...
    async create(request, response) {
        if (global.config.system === "rocket") {
            if (global.users.filter(u => !u.admin).length > 0 && !(await User.validateToken(request.headers.authorization))) {
                return response.send({
                    token: false,
                    error: "Unauthorized."
                });
        ...
        if (global.config.system === "rocket") {
            if (global.users.filter(u => !u.admin).length > 0 && !global.admin) {
                return response.send({
                    token: false,
                    error: "Unauthorized."
                });
