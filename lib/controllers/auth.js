...
module.exports = class AuthController {
    ...
    state(request, response) {
        if (HBS.config.system === "rocket") {
            if (HBS.users.filter(u => !u.admin).length === 0) {
    ...
    async create(request, response) {
        if (HBS.config.system === "rocket") {
            if (HBS.users.filter(u => !u.admin).length > 0 && !(await User.validateToken(request.headers.authorization))) {
                return response.send({
                    token: false,
                    error: "Unauthorized."
                });
        ...
        if (HBS.config.system === "rocket") {
            if (HBS.users.filter(u => !u.admin).length > 0 && !HBS.admin) {
                return response.send({
                    token: false,
                    error: "Unauthorized."
                });
