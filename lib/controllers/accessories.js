...
module.exports = class AccessoriesController {
    ...
    listRooms(request, response) {
        this.fetchServices().then((accessories) => {
            ...
            const layout = global.layout[username] || {
                rooms: [],
                hidden: [],
                names: {}
            };
            
            const current = JSON.parse(JSON.stringify(layout));
                ...
                for (let j = 0; j < current.rooms[i].accessories.length; j++) {
                    ...
                    if (placed.indexOf(parseFloat(aid)) === -1 && lookup[aid]) {
                        for (let k = 0; k < lookup[aid].length; k++) {
                            if ((layout.names || {})[aid] && (layout.names || {})[aid] !== "") {
                                this.rooms[lookup[aid][k]].alias = layout.names[aid];
                            }
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (placed.indexOf(parseFloat(aid)) === -1 && lookup[aid]) {
                    for (let j = 0; j < lookup[aid].length; j++) {
                        if ((layout.names || {})[aid] && (layout.names || {})[aid] !== "") {
                            this.rooms[lookup[aid][j]].alias = layout.names[aid];
                        }
                ...
                for (let i = 0; i < current.favorites.length; i++) {
                    ...
                    if (items) {
                        for (let j = 0; j < items.length; j++) {
                            ...
                            if ((layout.names || {})[aid] && (layout.names || {})[aid] !== "") {
                                this.accessories[items[j]].alias = layout.names[aid];
                            }
    ...
    listAccessories(request, response) {
        this.fetchServices().then((accessories) => {
            ...
            for (let i = 0; i < this.accessories.length; i++) {
                ...
                if ((layout.names || {})[aid] && (layout.names || {})[aid] !== "") {
                    this.accessories[i].alias = layout.names[aid];
                }
            }
