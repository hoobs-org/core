...
module.exports = class AccessoriesController {
    constructor() {
        ...
        global.app.get("/api/accessories/available", (request, response) => this.available(request, response));
    ...
    available(request, response) {
        ...
        this.fetchServices().then((data) => {
            ...
            global.log.error(`[Available Accessories] ${error.message}`);
        }).finally(() => {
            const lookup = {};
            const current = this.getCurrentUser(request);
            const placed = current.hidden.slice(0);
    ...
    getCurrentUser(request) {
        ...
        const current = global.layout[username] || {
            rooms: [],
            hidden: [],
            names: {}
        };
