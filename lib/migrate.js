...
module.exports = class Migrate {
    ...
    static layout(path) {
        ...
        console.log(`Migrating config '${join(storage, "accessories/layout.json")}'.`);
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            for (let j = 0; j < layout[keys[i]].length; j++) {
                const item = layout[keys[i]][j];
                ...
                for (let k = 0; k < item.services.length; k++) {
                    let aid = parseFloat(item.services[k].aid);

                    if (!Number.isNaN(aid)) {
                        let count = 0;
                        let safety = 0;

                        while (room.accessories.indexOf(aid) >= 0) {
                            count += 1;
                            safety += 1;

                            aid = parseFloat(`${item.services[k].aid}.${count}`);
                        ...
                        if (!Number.isNaN(aid)) {
                            if (item.services[k].hidden) {
                                layout[keys[i]].hidden.push(aid);
                            } else {
                                room.accessories.push(aid);

                                if (item.services[k].customName && item.services[k].customName !== "") {
                                    layout[keys[i]].names[aid] = item.services[k].customName;
                                }
                            }
                        }
                    }
