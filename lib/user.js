...
module.exports = class User {
    static async generateToken(id, remember) {
        ...
        if (user) {
            const token = {
                ...
                admin: user.admin,
                ttl: remember ? 525600 : global.config.client.inactive_logoff,
                token: await User.hashValue(user.password, key)
            };

            global.cache.set(Buffer.from(JSON.stringify(token), "utf8").toString("base64"), true, token.ttl);
    ...
    static get(username) {
        return global.users.filter(u => u.username.toLowerCase() === username.toLowerCase())[0];
    }
    ...
    static async validateToken(token) {
        ...
        const server = global.cache.get(token);
        ...
        try {
            ...
            if (challenge === data.token) {
                ...
                global.cache.set(token, true, data.ttl || global.config.client.inactive_logoff || 30);
