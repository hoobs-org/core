...
module.exports = async (docker) => {
    ...
    execSync(`rm -f ${join(Server.paths.application, "restore-*.zip")}`);
    execSync(`rm -f ${join(Server.paths.dist, "backup-*.hbf")}`);
    ...
    switch (HBS.mode) {
        case "server":
            ...
            process.once("uncaughtException", async (error) => {
                if ((error.message || "").toLowerCase().indexOf("cannot find module") >= 0 && File.existsSync(join(Server.paths.application, "dist"))) {
                    File.unlinkSync(join(Server.paths.application, "dist"))
                }
